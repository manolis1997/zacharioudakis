...
/*Store Procedure about the name of doctor for one patient*/

DELIMITER $$
CREATE PROCEDURE patient(in p int,out f varchar(100),out l varchar(100))
BEGIN
declare x int;

select giatros into x from asthenis where id=p;
select f_n,l_n into f,l from giatros where x=id;

END$$
DELIMITER $$

call patient(103,@f,@l);
select @f,@l;


/*Create function max patients for all doctors*/

DELIMITER$$
CREATE FUNCTION doc() returns int reads sql data
BEGIN
declare x int;

select count(giatros) into x from asthenis group by giatros order by 1 desc limit 1;
